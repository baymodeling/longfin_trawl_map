...
#     xy_leg = [589782.5317557553,4187778.3682879894] # move to main, Legend coords
    ...
    if plotType == 'Log':
        legend_data = {"med": 10**np.mean([np.log10(plot_range[1]), np.log10(plot_range[0])]), #make a dummy legend with perfect data
                       "q1": 10**(np.mean([np.log10(plot_range[1]), np.log10(plot_range[0])]) * .5),
                       "q3": 10**(np.mean([np.log10(plot_range[1]), np.log10(plot_range[0])]) * 1.5),
                       "whislo": plot_range[0],
                       "whishi": plot_range[1]}
    else:
    ...
    plotBWPlot(fig_coords, legend_array, fig, plot_range,
                 ...
                 xlabels=boxes, ylabel=leg_args['ylabel'], plotType=plotType)
        ...
        if len(tot_labels) > 0:
            if len(tot_labels) > 10:
                for tick, label in zip(range(numBoxes), tot_labels):
                    ax.text(pos[tick], y_range[0] * 1.005, label, ha='center', va='bottom', color='red', fontsize=5)
            else:
                for tick, label in zip(range(numBoxes), tot_labels):
...
#         ind = np.arange(5, (len(tot_box_vals) + 1)* 5, 5)
        ...
        if plotType == 'Log':
            ax.set_yscale('log')
#             ax.set_yticks([y_range[0], y_range[1]])
            ax.set_ylim(y_range[0], y_range[1])
            ax.set_ylabel(ylabel)

            yticks = [y_range[0]]
            ytick_lab = ['$10^{{{0}}}$'.format(int(np.log10(y_range[0])))]
            y_tick_mid = 10**np.mean([np.log10(y_range[1]), np.log10(y_range[0])])
            yticks.append(y_tick_mid)
            if np.log10(y_tick_mid).is_integer():
                ylab_mid = int(np.log10(y_tick_mid))
            else:
                ylab_mid = round(np.log10(y_tick_mid), 1)
            ytick_lab.append('$10^{{{0}}}$'.format(ylab_mid))
            yticks.append(y_range[1])
            ytick_lab.append('$10^{{{0}}}$'.format(int(np.log10(y_range[1]))))
            
            #works but too many ticks...
#             y_exp = np.log10(y_range[0])
#             while y_exp <= np.log10(y_range[1]):
#                 yticks.append(10**y_exp)
#                 ytick_lab.append('10$^%d$'%(y_exp))
#                 y_exp += 1
